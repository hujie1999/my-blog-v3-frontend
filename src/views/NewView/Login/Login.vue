          ...
          .then((res) => {
            ...
            if (temp.code == 1000) {
              let userinfo = {
                ...
                // avatar: setting.imgbaseurl+temp.avatar
                avatar: temp.avatar,
              };
              this.$store.commit("modifyUserInfo", userinfo);
              ...
              setTimeout(() => {
                this.btnlock = false;
                this.$router.replace({ name: "BaseLayout" });
              }, 3000);
            ...
            // else if (temp.code == 1005) {
            //   this.$notify({
            //     title: '通知',
            //     message: "登陆失败，该账号正处于审核状态！\n如有需要，请联系管理员加快审核进度！",
            //     type: "warning",
            //   });
            //   this.btnlock = false
            // }
            else if (temp.code == 1006) {
              this.$notify({
                title: "失败",
                message: "登陆失败，该账号未通过管理员审核！",
                type: "error",
                duration: 5000,
              });
              this.btnlock = false;
