<template>
  <div id="comments-area">
    ...
    <!-- 回复 不为空时展现内容  -->
    <div v-show="information.length != 0">
      <!-- 第一层评论 -->
            ...
            <div class="comment-area">
              <div class="comment">
                <div class="nickname">
                  ...
                  v-if="items['Comment_Person_Id']==Blog_Author_UniqueId"
                  >
                    作者
                  </span>
                ...
                <div class="comment-area">
                  <div class="comment">
                    <div class="nickname">
                      ...
                      <span class="identity-tag is-author"
                      v-show="it['Comment_Person_Id']==Blog_Author_UniqueId"
                      >
                        作者
                      </span>
                      <span class="identity-tag is-admin"
                      ...
                      <span class="identity-tag is-author"
                      v-show="it['Parent_Person_Id']==Blog_Author_UniqueId"
                      >
                        作者
                      </span>
                      <span class="identity-tag is-admin"
                  ...
                  <div class="opration">
                    <div class="reply-area" @click="wirteReply({
                        Blog_Id: '',
                        Blog_Title: '',
                        Comment_Level: '',
                        Comment_Id: '',
...
export default {
  ...
  props: {
    ...
    Blog_Title: "",
    Blog_Author_Role:"",
    Blog_Author_UniqueId:"",
    Blog_Author_Account:"",
    Blog_Author_NickName:"",
    Blog_Author_Avatar:"",
    transWriteComment: {
      type: Function,
    },
  ...
  methods: {
    ...
    //评论后，评论数+1
    commentCountPlusOne(){
      addcommentcountAPI({Blog_Id:this.Blog_Id}).then(res=>{}).catch(err=>{})
    },
    //回复成功
    replySuccessed(){
      this.$message({
    ...
    writeComment() {
      ...
      let presend = {
        ...
        Parent_Person_Role: this.Blog_Author_Role,
        Parent_Person_Acc: this.Blog_Author_Account,
        Parent_Person_Name: this.Blog_Author_NickName,
        Parent_Person_Id: this.Blog_Author_UniqueId,
        Parent_Person_Avatar: this.Blog_Author_Avatar,
        Comment_Content: this.textarea,
      };
      // console.log(presend)
      writecommentandreplyAPI({ Information: presend })
        .then((res) => {
          if (res.data.code == 1012) {
            // console.log("评论成功");
            this.replySuccessed()
            this.$emit("transWriteComment",0);
            this.writecomment_locker = false;
            this.textarea = "";
            this.commentCountPlusOne()
            this.all+=1
    ...
    handleReply(reply) {
      ...
      setTimeout(()=>{
        writecommentandreplyAPI({ Information: this.reply_info})
        .then((res) => {
          if (res.data.code == 1012) {
            // console.log("回复成功");
            this.replySuccessed()
            this.handleCancel()
            this.$emit("transWriteComment",this.reply_level);
